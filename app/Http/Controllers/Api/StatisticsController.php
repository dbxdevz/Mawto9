...
class StatisticsController extends Controller
{
    public function index(Request $request)
    {
        ...
        $status               = OrderStatus::where('status', 'Confirmed')
                                           ->first()
        ;
        ...
        if ($topProduct) {
            $OrderProductId     = OrderDetail::where('product_id', $topProduct->id)
                                             ->first()
            ;
    ...
    private function getCount($modal, $start, $end, $modal_id, $modal_name)
    {
        ...
        $modal_id == null and $modal_name == null
            ?
            $count = $modal::whereBetween('created_at', [$start . " 00:00:00", $end . " 23:59:59"])
                           ->count()
            :
    ...
    {
        $topModals = $modal::withCount($relation)
                           ->whereHas($relation, function ($query) use ($start, $end, $relation) {
                               return $query->whereBetween(
                                   $relation . '.created_at', [$start . " 00:00:00", $end . " 23:59:59"]
                               );
                           })
                           ->get()
        ;
