...
class DeliveryManController extends Controller
{
    ...
    {
        $deliveryMen = User::whereHas('deliveryInfo')->get();
    ...
    public function store(StoreDeliveryManRequest $request)
    {
        ...
        $user->roles()->attach(Role::whereName('Courier')->first());
    ...
    {
        $deliveryMan = User::find($deliveryMan->user_id)->load('deliveryInfo');
    ...
    public function toggleDefault(ToggleDefaultDeliveryManRequest $request, $id)
    {
        if ($request->get('default')) {
            DB::table('delivery_men')->update(['default' => false]);
        }

        $deliveryMan = DeliveryMan::where('user_id', $id)->first();
